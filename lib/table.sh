...
p6_pgsql_filter_pipe_to_table() {
    ...
    while read -r line; do
        if ! p6_string_blank "$line"; then
            local values=$(p6_echo "$line" | p6_filter_translate_blank_to_null | p6_filter_sql_escape_single_quote | p6_filter_translate_words_to_sql_list "$sep" | p6_filter_translate_quoted_null_to_null)
